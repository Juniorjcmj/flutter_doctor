...
class PageDetailPaciente extends StatefulWidget {
  ...
  PageDetailPaciente({Key? key, required this.pacienteDetail})
      : super(key: key);
...
class _PageDetailPacienteState extends State<PageDetailPaciente> {
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () async {
              var response = await Get.to(
                  PacienteForm(paciente: pacienteController.paciente.value));
              Paciente result = response['Paciente'] as Paciente;
              pacienteController.atualizarPaciente(result);

              Get.snackbar("Parabéns", "Dados Atualizados com sucesso!",
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Obx(() =>
                _buildInfo('Nome', pacienteController.paciente.value.nome)),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Obx(() =>
                    _buildInfo('CPF', pacienteController.paciente.value.cpf)),
                Obx(() => _buildInfo('Idade',
                    pacienteController.paciente.value.idade.toString())),
              ],
            ),
            ...
            Row(
              ...
              children: [
                Obx(() => _buildInfo(
                    'Convênio', pacienteController.paciente.value.convenio)),
                Obx(() => _buildInfo('Carteirinha',
                    pacienteController.paciente.value.carteirinha)),
              ],
            ),
            Obx(() =>
                _buildInfo('Gênero', pacienteController.paciente.value.genero)),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Obx(() => _buildInfo(
                    'Whatsapp', pacienteController.paciente.value.whatsapp)),
                Obx(() => _buildInfo(
                    'Celular', pacienteController.paciente.value.celular)),
              ],
            ),
            ...
            Row(
              ...
              children: [
                Obx(() => _buildInfo(
                    'Bairro', pacienteController.paciente.value.bairro)),
                Obx(() => _buildInfo(
                    'Cidade', pacienteController.paciente.value.cidade)),
                Obx(() =>
                    _buildInfo('UF', pacienteController.paciente.value.uf)),
              ],
            ),
  ...
  Future<void> _confirmarExclusao(BuildContext context) async {
    ...
    if (confirmado == true) {
      bool result = await pacienteController
          .deletar(pacienteController.paciente.value.id.toString());
      if (result) {
        Get.snackbar("OK", "Paciente Excluido com sucesso!",
            backgroundColor: Config.primaryColor,
        ...
        Get.snackbar("Error",
            "Ocorreu problema ao tentar Excluir, entre em contato com administrador!",
